...
/** Borde + halo rojo para gráficas y tablas (no aplica a las 4 tarjetas principales). */
const DASHBOARD_CARD_GLOW =
  "border border-red-600/45 shadow-[0_0_0_1px_rgba(220,38,38,0.42),0_0_36px_rgba(185,28,28,0.22),0_0_56px_-10px_rgba(127,29,29,0.14)] ring-1 ring-red-500/35";
...
function InteractionsTimelineChart({
  ...
  if (rows.length === 0) {
    return (
      <div className="flex min-h-[15rem] items-center justify-center text-sm text-white/65">
        Sin datos aún
      </div>
    );
  }
    ...
    <div className="space-y-4 pt-1">
      {rows.map((row) => (
        <div key={row.id}>
          ...
          >
            <div
              className="h-full rounded-full bg-gradient-to-r from-red-800/90 to-red-500/85"
              style={{
...
function EventDistributionRadial({
  ...
  if (total === 0) {
    return (
      <div className="flex h-52 min-w-[200px] items-center justify-center text-sm text-white/65">
        Sin eventos
      </div>
    );
  }
      ...
      >
        ...
        >
          ...
          <text
            ...
            y={cy - 4}
            textAnchor="middle"
            className="fill-white text-[17px] font-semibold drop-shadow-[0_1px_2px_rgba(0,0,0,0.45)]"
          ...
          <text
            ...
            y={cy + 14}
            textAnchor="middle"
            className="fill-white/65 text-[9px] uppercase tracking-wider drop-shadow-[0_1px_1px_rgba(0,0,0,0.35)]"
            style={{ fontFamily: "inherit" }}
          >
            eventos
          </text>
    ...
    <div className="space-y-8">
      <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        ...
        <div className="mt-3 flex flex-row flex-wrap items-center justify-center gap-3 sm:mt-5 sm:justify-end sm:gap-4">
          ...
          <div
            className="inline-flex rounded-lg border border-red-600/40 bg-muted/25 p-0.5 shadow-inner shadow-black/20"
            role="group"
          ...
          >
            <button
              ...
              className={cn(
                ...
                metricsRange === "7d"
                  ? "bg-red-600/25 text-red-50 shadow-sm ring-1 ring-red-400/50"
                  : "text-red-200/70 hover:text-red-50",
              )}
            >
              Últimos 7 días
            </button>
      ...
      <div className="grid min-w-0 gap-6 lg:grid-cols-2 lg:items-stretch">
        <Card
          className={`flex min-h-0 h-full min-w-0 flex-col text-white ${DASHBOARD_CARD_GLOW} bg-gradient-to-b from-card to-card/50`}
        >
          <CardHeader className="shrink-0 pb-2">
            <CardTitle className="uppercase tracking-wide text-white">
          ...
          <CardContent className="flex min-h-0 flex-1 flex-col overflow-x-auto pt-0 pb-5">
            ...
            <p className="-mt-1 mb-0.5 text-center text-[11px] font-medium tracking-wide text-white/65">
              Día
            </p>
            <div
            ...
            >
                {areaChartRows.map((row) => (
                  ...
                  >
                    ...
                    <span className="text-[10px] tabular-nums leading-none text-white/55">
                      {row.label}
                    </span>
                  </div>
                ))}
      ...
      <div className="grid min-w-0 gap-6 lg:grid-cols-2">
        <Card className={`min-w-0 text-white ${DASHBOARD_CARD_GLOW} bg-card/80`}>
          <CardHeader>
            <CardTitle className="uppercase tracking-wide text-white">
          ...
          <CardContent className="pt-2">
            {(data?.recentEvents?.length ?? 0) === 0 ? (
              <div className="flex min-h-48 items-center justify-center px-4 py-10">
                <p className="text-sm text-white/65">Aún no hay eventos</p>
              </div>
            ) : (
      ...
      <Card className={`min-w-0 text-white ${DASHBOARD_CARD_GLOW} bg-card/80`}>
        ...
        <CardContent className="overflow-x-auto pt-2">
          <table className="w-full min-w-[640px] text-left text-sm">
            <thead>
              <tr className="border-b border-border text-white/65">
            ...
            <tbody>
              <tr className="border-b-2 border-border bg-muted/25 text-white">
                ...
                <td className="py-3 pl-2 text-center tabular-nums font-semibold text-white">
              ...
              {detailTableRows.map((row) => {
                ...
                return (
                  <tr
                    ...
                    className="border-b border-border/50 text-white/95 last:border-0"
                  >
