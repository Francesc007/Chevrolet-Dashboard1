...
/** Año en columna `year`; acepta número o null desde JSON. */
...
export async function POST(request: Request) {
  ...
  try {
    const supabase = createAdminClient();
    const { model: modelName, year: yearVal } = resolveReviewVehicleFields({
      model: v.model,
    ...
    if (result.error && isMissingExtendedReviewColumns(result.error)) {
      const legacyRow = {
        ...
        model:
          legacyModelLine(modelName, yearVal) ?? modelName,
