...
create table if not exists public.cars (
  ...
  constraint gallery_max_five check (coalesce(cardinality(gallery_urls), 0) <= 5)
);
...
-- Tiempo real para el dashboard (idempotente si la tabla ya está en la publicación)
do $$
begin
  alter publication supabase_realtime add table public.landing_interactions;
exception
  when others then
    if sqlerrm ilike '%already%' and sqlerrm ilike '%publication%' then
      null;
    else
      raise;
    end if;
